...
export default function RepoCard({ repoProps }: { repoProps: GithubRepos }) {
  ...
  return (
    <div className="w-full grid grid-cols-3 px-2 py-2 place-items-center text-secondary-text lg:grid-cols-[1fr_1.5fr_1fr_1fr_1fr_1fr] lg:grid-rows-1 lg:gap-1 lg:text-center truncate">
      <div className="max-w-full sm:col-start-1 truncate" role="cell">
      ...
      <div className="sm:col-start-3" role="cell">
        <a href={html_url}>{language ?? "Not informed"}</a>
      </div>
      <div className="hidden truncate sm:block sm:col-start-4" role="cell">
        <a href={html_url}>{watchers_count}</a>
      </div>
      <div className="hidden truncate sm:block sm:col-start-5" role="cell">
        <a href={html_url}>{stargazers_count}</a>
      </div>
      <div className="sm:col-start-6" role="cell">
        <a href={html_url}>{formattedDate}</a>
      </div>
    </div>
  );
}
